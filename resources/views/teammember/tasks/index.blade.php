            ...
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h5 class="text-white text-capitalize mb-0">
                        @if(Auth::user()->hasRole(['Developer', 'QA']))
                            Tugas Saya
                        @else
                            Daftar Tugas
                        @endif
                    ...
                    <div class="d-flex gap-2">
                        <div class="dropdown">
                            <button class="btn btn-outline-light btn-sm dropdown-toggle mb-0" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                <i class="fas fa-filter me-1"></i>
                                {{ request('status') ? Str::title(str_replace('-', ' ', request('status'))) : 'Filter Status' }}
                            </button>
                            <ul class="dropdown-menu">
                                <li><a class="dropdown-item" href="{{ route('teammember.tasks.index', request()->except('status', 'page')) }}">Semua Status</a></li>
                                @foreach ($statusesForFilter as $status)
                                    <li>
                                        <a class="dropdown-item" href="{{ route('teammember.tasks.index', array_merge(request()->except('page'), ['status' => Str::slug($status)])) }}">
                                            {{ $status }}
                                        </a>
                                    </li>
                                @endforeach
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                <li><a class="dropdown-item" href="{{ route('teammember.tasks.index', array_merge(request()->except('page'), ['sort' => 'due_date'])) }}">Tenggat Waktu</a></li>
                                <li><a class="dropdown-item" href="{{ route('teammember.tasks.index', array_merge(request()->except('page'), ['sort' => 'priority'])) }}">Prioritas</a></li>
                            </ul>
                        </div>
                    </div>
                </div>

                {{-- BLOK STATISTIK YANG SUDAH DIPERBAIKI --}}
                <div class="row mb-4">
                    @if(Auth::user()->hasRole(['Developer', 'QA']))
                                    ...
                                    <div class="d-flex align-items-center mb-3">
                                        <div>
                                            <h6 class="card-title fw-bold mb-0">
                                                <a href="{{ route('teammember.tasks.show', $task) }}" class="text-dark text-decoration-none stretched-link">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div>
                                                @php
                                                    ...
                                                    if ($task->status === 'In Progress') $status_class = 'bg-info';
                                                    if ($task->status === 'Completed') $status_class = 'bg-success';
                                                    if ($task->status === 'Revisi') $status_class = 'bg-warning';
                                                    if ($task->status === 'Blocked') $status_class = 'bg-danger';
                                                    if ($task->status === 'In Review') $status_class = 'bg-primary';
                                                @endphp
...
@push('styles')
    <style>
        ...
        .task-card:hover { transform: translateY(-5px); box-shadow: 0 10px 20px rgba(0, 0, 0, 0.12), 0 4px 8px rgba(0, 0, 0, 0.06); }
        .avatar-group .avatar { margin-left: -8px; border: 2px solid #fff; transition: transform 0.2s ease; }
        .avatar-group .avatar:hover { transform: scale(1.1) translateY(-2px); z-index: 10; }
