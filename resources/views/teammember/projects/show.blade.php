                        ...
                        <div class="table-responsive p-0">
                            <table class="table align-items-center mb-0">
                                <tbody>
                                    @forelse($myTasks as $task)
                                        <tr>
                                                ...
                                                <div class="d-flex px-3 py-1">
                                                    <div class="d-flex flex-column justify-content-center">
                                                        <h6 class="mb-0 text-sm">
                                                            <a href="{{ route('teammember.tasks.show', $task->id) }}"
                                                                class="text-dark fw-bold text-decoration-none">{{ $task->title }}</a>
                                                        </h6>
                                                            ...
                                                            <div class="mt-2">
                                                                @foreach ($task->attachments as $attachment)
                                                                    ...
                                                                    <a href="{{ Storage::url($attachment->file_path) }}"
                                                                        target="_blank"
                                                                        class="text-xs me-3 text-decoration-none">
                                                                        <i
                                                                            class="fas fa-paperclip text-secondary me-1"></i>{{ $attachment->file_name ?? 'Lampiran' }}
                                                                    </a>
                                                                @endforeach
                                                            </div>
                                                        @endif
                                            ...
                                            <td class="align-middle text-center text-sm">
                                                ...
                                                @endphp
                                                <span
                                                    class="badge badge-sm {{ $status_class }}">{{ $task->status }}</span>
                                            </td>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="3" class="text-center py-4 text-muted">Anda tidak memiliki tugas
                                                pada proyek ini.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                        ...
                        <ul class="list-group list-group-flush">
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                                ...
                                @php
                                    ...
                                    if ($project->priority == 'Sedang') {
                                        $priority_color = 'bg-gradient-warning';
                                    }
                                    if ($project->priority == 'Rendah') {
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                                    <div class="d-flex align-items-center">
                                        <img src="{{ $member->profile_photo_url ?? 'https://placehold.co/40x40/EFEFEF/333333?text=' . substr($member->name, 0, 1) }}"
                                            alt="{{ $member->name }}" class="avatar avatar-sm rounded-circle me-3">
                                        <div>
                                            <h6 class="mb-0 text-sm">{{ $member->name }}</h6>
                                            <p class="text-xs text-secondary mb-0">{{ $member->role?->name ?? 'Member' }}
                                            </p>
                                        </div>
                                    </div>
