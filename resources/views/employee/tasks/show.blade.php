                ...
                <div class="d-flex justify-content-between align-items-center px-3">
                    ...
                    <div class="d-flex gap-2">
                        @if($task->status !== 'Selesai')
                            <a href="{{ route('employee.task-updates.create', $task->id) }}" class="btn btn-light btn-sm mb-0">
                                <i class="fas fa-plus me-1"></i> Tambah Update
                            </a>
                        @endif
                        <a href="{{ route('employee.tasks.index') }}" class="btn btn-outline-light btn-sm mb-0">
                ...
                <div class="card-header p-0">
                    <ul class="nav nav-tabs nav-justified" id="taskTab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="detail-tab" data-bs-toggle="tab" data-bs-target="#detail-pane" type="button" role="tab">Detail</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="attachment-tab" data-bs-toggle="tab" data-bs-target="#attachment-pane" type="button" role="tab">Lampiran</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="discussion-tab" data-bs-toggle="tab" data-bs-target="#discussion-pane" type="button" role="tab">Diskusi</button>
                        </li>
                    </ul>
                </div>
                <div class="card-body">
                                ...
                                <ul class="list-group list-group-flush">
                                    @foreach($task->attachments as $attachment)
                                        <li class="list-group-item d-flex justify-content-between align-items-center px-0 border-0">
                                            ...
                                            <a href="{{ Storage::url($attachment->file_path) }}" target="_blank" class="btn btn-sm btn-outline-dark mb-0">Download</a>
                                        </li>
                                    @endforeach
                                </ul>
                            @else
                        ...
                        <div class="tab-pane fade" id="discussion-pane" role="tabpanel">
                            ...
                            @forelse($task->comments->sortByDesc('created_at') as $comment)
                                <div class="d-flex mt-3">
                                    <img src="{{ $comment->user->profile_photo_url ?? 'https://placehold.co/40x40/EFEFEF/333333?text='.substr($comment->user->name, 0, 1) }}" class="avatar avatar-sm rounded-circle me-3">
                        ...
                        <div class="timeline-block mb-3">
                            ...
                            <div class="timeline-content">
                                ...
                                <p class="text-secondary font-weight-bold text-xs mt-1 mb-0">
                                    {{ $update->created_at->isoFormat('D MMM Y, HH:mm') }} oleh {{ $update->user?->name }}
                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                            ...
                            @php
                                ...
                                if ($priority == 'Tinggi') $priority_color = 'bg-gradient-danger';
                                if ($priority == 'Sedang') $priority_color = 'bg-gradient-warning';
                                if ($priority == 'Rendah') $priority_color = 'bg-gradient-info';
                            @endphp
...
{{-- Menambahkan style khusus untuk timeline --}}
@push('styles')
<style>
.timeline-block {
    position: relative;
